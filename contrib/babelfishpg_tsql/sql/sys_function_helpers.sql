    ...
    BEGIN
        ...
        ELSE
            ...
            IF (v_res_datetime > make_timestamp(9999, 12, 31, 23, 59, 59.999999)) THEN
                -- if rounding of fractional seconds caused the date and time to go out of range 
                -- then max date and time that can be stored for p_datatype will be used
                v_res_datetime := make_timestamp(9999, 12, 31, 23, 59, pg_catalog.concat_ws('.', '59', PG_CATALOG.repeat('9', LEAST(v_scale, 6)))::NUMERIC); 
            END IF;
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    ...
    IF (v_res_time > make_time(23, 59, 59.999999)) THEN
        ...
        v_res_time := make_time(23, 59, pg_catalog.concat_ws('.', '59', PG_CATALOG.repeat('9', LEAST(v_scale, 6)))::NUMERIC); 
    END IF;
